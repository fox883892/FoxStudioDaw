...
void MainComponent::resized()
{
    ...
    inspectorLabel.setBounds(right.removeFromTop(34).reduced(10, 4)); tempoSlider.setBounds(right.removeFromTop(48).reduced(10)); masterSlider.setBounds(right.removeFromTop(48).reduced(10)); loopButton.setBounds(right.removeFromTop(32).reduced(10, 4)); metronomeButton.setBounds(right.removeFromTop(32).reduced(10, 4));
}
...
void MainComponent::resetSession() { playing = false; positionSeconds = 0.0; trackCount = 0; playButton.setButtonText("Play"); trackLabel.setText("No tracks", juce::dontSendNotification); statusLabel.setText("New session", juce::dontSendNotification); }
juce::String MainComponent::formatTime(double seconds) const { const auto ms = (int)std::round(seconds * 1000.0); return juce::String::formatted("%02d:%02d.%03d", ms / 60000, (ms / 1000) % 60, ms % 1000); }
